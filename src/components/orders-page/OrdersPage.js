...
import {
  collection, doc, getDoc, getDocs, increment, query, setDoc, where,
...
function OrdersPage() {
  ...
  const handleSubmit = async () => {
    ...
    const itemRef = doc(db, `dishes/${productId}`);
    try {
      await setDoc(itemRef, {
  ...
  return (
    <div className="content">
      ...
      {
				backVisible ? (
  <div className="link-panel" onClick={showOrdersList}>
    <div className="payment-link-wrapper">
      <img src={back} alt="back" />
    </div>
  </div>
  ...
  <div className="product-rating">
    ...
    <img
      style={{
        width: '200px', height: '150px', marginLeft: '100px', borderRadius: '10px',
      }}
      src={dishDetailImage}
      alt="img"
    />
  ...
  <div key={userOrder.id} className="order-wrapper">
    ...
    <div className="order-description">
      ...
      <div
        ...
        onClick={() => showOrder(userOrder.id)}
      >
        Order again
      </div>
    </div>
