...
const CardDetailsPage = () => {
	...
	const updateCardDetails = async () => {
		...
		if (nameOnCard.length === 0) {
			...
			let cardId = new Date().toISOString();
			let userRef = doc(db, `users/${userId}/userCard/${cardId}`);
			if (cardNumber[0] == '4') {
	...
	return (
		<div className="content">
			...
			<div className="card-details-wrapper">
				<input className="input-log width-295 cart-details-input"
				...
				<div className="input-group">
					<input className="input-log cart-details-input"
				...
				<button onClick={updateCardDetails} className="btn btn-295">UPDATE</button>
			</div>
		</div>
	)
}
